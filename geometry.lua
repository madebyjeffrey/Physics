
vector = require "hump.vector"
Class = require "hump.class"

Rectangle = Class{name = "Rectangle",
				  function(self, origin, size)
					self.origin = origin
					self.size = size
				  end}
					
function Rectangle:hitTest(self, object)
	if object:is_a(Circle) then
		local closest = self:clamp(object.origin)
		local distance = (object.origin - closest):len2()
		
		return distance < (object.radius*object.radius)
	end
	
	return false
end

function Rectangle:left(self)
	return self.origin.x
end

function Rectangle:top(self)
	return self.origin.y
end

function Rectangle:right(self)
	return self.origin.x + self.size.x - 1
end

function Rectangle:bottom(self)
	return self.origin.y + self.size.y - 1
end

function Rectangle:clamp(self, point)
	if vector.isvector(point) then
		return vector.new(math.clamp(point.x, self:left(), self:right()),
		                  math.clamp(point.y, self:top(), self:bottom()))
	end
end

function Rectangle:draw(self)
	graphics.setColor(self.colour)
	graphics.rectangle("line", self.position.x + 0.5, 
						       self.position.y + 0.5, 
						       self.size.x, self.size.y)
end

Circle = Class{name = "Circle",
			   function(self, origin, radius)
			     self.origin = origin
			     self.radius = radius
			   end}
			   
function Circle:draw(self)
	graphics.setColor(self.colour)
	graphics.circle("fill", self.origin.x + 0.5,
	                        graphics - self.origin.y + 0.5,
	                        self.radius)
end